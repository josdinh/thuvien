...
class CV_Thuvien_Block_Adminhtml_Khosach_Edit extends Mage_Adminhtml_Block_Widget_Form_Container
{
    ...
    {    	
        if( Mage::registry('khosach_data') && Mage::registry('khosach_data')->getId() ) {
        	$arrDetail = Mage::registry('khosach_data')->getData();
            return Mage::helper('thuvien')->__("Cập nhật thông tin kho sách '%s'", $this->htmlEscape($arrDetail['KhoSach']));
        } else {
            return Mage::helper('thuvien')->__('Thêm kho sách mới');
        }
    }
}