...
class CV_Thuvien_Block_Adminhtml_Theloai_Edit extends Mage_Adminhtml_Block_Widget_Form_Container
{
    ...
    {    	
        if( Mage::registry('theloai_data') && Mage::registry('theloai_data')->getId() ) {
        	$arrDetail = Mage::registry('theloai_data')->getData();
            return Mage::helper('thuvien')->__("Cập nhật thông tin thể loại tác phẩm '%s'", $this->htmlEscape($arrDetail['TheLoai']));
        } else {
            return Mage::helper('thuvien')->__('Thêm thể loại tác phẩm mới');
        }
    }
}