...
class CV_Thuvien_Block_Adminhtml_Quydinh_Edit extends Mage_Adminhtml_Block_Widget_Form_Container
{
    ...
    {    	
        if( Mage::registry('quydinh_data') && Mage::registry('quydinh_data')->getId() ) {
        	$arrDetail = Mage::registry('quydinh_data')->getData();
            return Mage::helper('thuvien')->__("Cập nhật thông tin Thư viện '%s'", $this->htmlEscape($arrDetail['TenThuVien']));
        } else {
            return Mage::helper('thuvien')->__('Thêm thông tin Thư viện mới');
        }
    }
}